...
import Crypto.Schnorr                    (Msg, SchnorrSig, XOnlyPubKey, verifyMsgSchnorr)
...
instance FromJSON Tag where
    parseJSON (AesonTypes.Array v)
        | V.length v == 3 = case v V.! 0 of
            AesonTypes.String "p" -> ETag <$> ((,) <$> parseJSON (v V.! 1) <*> parseJSON (v V.! 2))
            AesonTypes.String "e" -> PTag <$> ((,) <$> parseJSON (v V.! 1) <*> parseJSON (v V.! 2))
            _        -> mzero
        | otherwise = mzero
    parseJSON _ = mzero
...
serializeEvent e =
    toStrict $ encode $ AesonTypes.Array $ fromList $ --- @todo this must be a Data.Vector (aka JSON Array)
        [ AesonTypes.Number 0
...
serializeTag (ETag (i, r)) = AesonTypes.Array $ fromList
    [ AesonTypes.String $ pack "e"
    , AesonTypes.String $ pack $ exportEventId i
    , AesonTypes.String r
    ]
serializeTag (PTag (p, r)) = AesonTypes.Array $ fromList
...
verifySignature e =
    case Schnorr.msg $ serializeEvent e of
        Just m ->
            Schnorr.verifyMsgSchnorr p s m
        Nothing ->
            False
    where
        p = pubKey e
